...
import MathUtils (isPalindrome, reverseInteger)

isLychrel x = isLychrel' x 50
  where
    isLychrel' _ 0 = True
    isLychrel' x n =
      if isPalindrome (x + (reverseInteger x))
      then False
      else isLychrel' (x + (reverseInteger x)) (n - 1)

lychrelNumbers = filter isLychrel [1..n]
  where
    n = 10^4 - 1

solve = fromIntegral $ length lychrelNumbers
